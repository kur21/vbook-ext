...
function execute(url, page) {
    if (!page) page = '1';
    let response = fetch(url + "/page/" + page + '/?m_orderby=latest');
    if (response.ok) {
        ...
        doc.select(".page-item-detail").forEach(e => {
            comiclist.push({
                ...
                cover: e.select("img.img-responsive").attr("data-lazy-src") || e.select("img.img-responsive").attr("data-src") || e.select("img.img-responsive").attr("src"),
